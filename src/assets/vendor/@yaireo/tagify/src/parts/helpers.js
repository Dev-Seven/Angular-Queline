...
export const sameStr = (s1, s2, caseSensitive) =>
  caseSensitive ? s1 == s2 : ("" + s1).toLowerCase() == ("" + s2).toLowerCase();

// const getUID = () => (new Date().getTime() + Math.floor((Math.random()*10000)+1)).toString(16)
export const removeCollectionProp = (collection, unwantedProps) =>
  collection.map((v) => {
    var props = {};
    for (var p in v) if (unwantedProps.indexOf(p) < 0) props[p] = v[p];
    return props;
  });
